...
with st.sidebar:
    ...
    try:
        ...
        if health.status_code == 200:
            st.success("الخدمة تعمل")
        else:
            st.warning("الخدمة تستجيب")
    except Exception:
        st.error("الخدمة غير متصلة. شغّل السيرفر أولاً.")
    ...
    if not message.strip():
        ...
        try:
            ...
            if response.status_code != 200:
                st.error(payload.get("error", "حدث خطأ غير متوقع."))
            else:
            ...
            st.error(f"تعذر الاتصال بالخدمة: {exc}")
